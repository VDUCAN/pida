...
//	$errorMsg[] = 'driving_license_doc';
    ...
    $db = new DbHandler();
    $res = $db->driverSetProfile($data); // echo $res;die;
    if ($res == 'UNABLE_TO_PROCEED') {
	$response["code"] = 1;
	$response["error"] = true;
	$response["message"] = "Unable to proceed";
...
$app->post('/login', function() use ($app) {
	...
	$response['data']['country_name'] = $res['country_name'];
	$response['data']['state_name'] = $res['state_name'];
	$response['data']['city_name'] = $res['city_name'];
 ...
 * url - /book_driver
 * method - POST
 * params - vehicle_type_id (mandatory),lat (mandatory),long (mandatory) , booking_address(mandatory) , cargo_type_id (mandatory) , cargo_type_notes ,  delivery_type_id (mandatory) , delivery_type_notes , pickup_date   
 * header Params - email (mandatory), password (mandatory)
 */
