...
/*        jQuery('#reset_button').click(function(){
            ...
            jQuery('#order_by').val('DeliveryTypeLocale.name ASC');
        }); */
...
$option_order = array(
    'DeliveryTypeLocale.name ASC' => 'Delivery Type Ascending',
    'DeliveryTypeLocale.name DESC' => 'Delivery Type Descending',
    'DeliveryType.status ASC, DeliveryTypeLocale.name ASC' => 'Status Ascending',
    'DeliveryType.status DESC, DeliveryTypeLocale.name ASC' => 'Status Descending',
    'DeliveryType.created ASC' => 'Created On Ascending',
    'DeliveryType.created DESC' => 'Created On Descending',
);
...
<section id="page-title">
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <h1 class="mainTitle pull-left">Delivery Type List</h1>
            <div class="row pull-right">
                <?php
                echo $this->Html->link('Add Delivery Type <i class="fa fa-plus"></i>',
                    array('plugin' => false, 'controller' => 'delivery_types', 'action' => 'add_edit', 'admin' => true),
...
<div class="container-fluid container-fullw bg-white">
    ...
    <div class="border-around margin-bottom-15 padding-10">
        <?php echo $this->Form->create('DeliveryType', array(
                'url' => array('controller' => 'delivery_types', 'action' => 'index', 'admin' => true), 'class' => 'form', 'role' => 'search', 'autocomplete' => 'off')
        ); ?>
        ...
        <div class="col-md-12">
            <?php
            echo $this->Html->link('Reset <i class="fa fa-times-circle"></i>', array('controller' => 'delivery_types','action'=>'reset_filter','admin' => true), array('title' => 'Reset', 'escape' => false, 'class' => 'btn btn-primary btn-wide pull-right'));
    ...
    <?php echo $this->Form->create('PageSize', array(
            'url' => array('controller' => 'delivery_types', 'action' => 'index', 'admin' => true), 'class' => 'form', 'autocomplete' => 'off', 'id' => 'pageSizeForm')
    ); ?>
            ...
            <table class="table table-striped table-bordered table-hover table-full-width">
                ...
                <?php if (!empty($result_data)) { ?>
                    <?php foreach ($result_data as $data) { ?>
                        <tr>
                            <td><?php echo ucfirst($data['DeliveryTypeLocale']['name']); ?></td>
                            <td><?php echo $languages[$data['DeliveryTypeLocale']['lang_code']]; ?></td>
                            <td> <?php
                                if ($data['DeliveryType']['status'] == 'A') {
                            ...
                            <td>
                                <?php echo date(DATETIME_FORMAT, $data['DeliveryType']['created']); ?>
                            </td>
                            <td>
                                <div>
                                    <?php
                                    if ($data['DeliveryType']['status'] == 'A') {
                                        echo $this->Html->link('<i class="fa fa-check-circle"></i>', array('controller' => 'delivery_types', 'action' => 'status', base64_encode($data['DeliveryType']['id']), 'I', 'admin' => true), array('title' => 'Click here to inactive', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                    } else {
                                        echo $this->Html->link('<i class="fa fa-times-circle"></i>', array('controller' => 'delivery_types', 'action' => 'status', base64_encode($data['DeliveryType']['id']), 'A', 'admin' => true), array('title' => 'Click here to active', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                    }

                                    echo $this->Html->link('<i class="fa fa-pencil"></i>',
                                        array('plugin' => false, 'controller' => 'delivery_types', 'action' => 'add_edit', 'id' => base64_encode($data['DeliveryType']['id']), 'admin' => true),
                                        array('class' => 'btn btn-transparent btn-xs', 'title' => 'Click here to edit Delivery Type', 'escape' => false)
                                    );
                                     ?>
                                </div>
                            </td>
                    ...
                    ?>
                    <tr>
                        <td colspan="5" class="text-center">Delivery Type Not Available !</td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
