...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        jQuery('#reset_button').click(function(){
            ...
            jQuery('#order_by').val('StateLocale.name ASC');
        });
...
$option_order = array(
    'StateLocale.name ASC' => 'State Name Ascending',
    'StateLocale.name DESC' => 'State Name Descending',
    'State.status ASC, StateLocale.name ASC' => 'Status Ascending',
    'State.status DESC, StateLocale.name ASC' => 'Status Descending'
);

$search_txt = $search_country = $status = $search_lang = '';
...
<section id="page-title">
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <h1 class="mainTitle pull-left">State List</h1>
            <div class="row pull-right">
                <?php
                echo $this->Html->link('Add State <i class="fa fa-plus"></i>',
                    array('plugin' => false, 'controller' => 'states', 'action' => 'add_edit', 'admin' => true),
...
<div class="container-fluid container-fullw bg-white">
    ...
    <div class="border-around margin-bottom-15 padding-10">
        <?php echo $this->Form->create('State', array(
                'url' => array('controller' => 'states', 'action' => 'index', 'admin' => true), 'class' => 'form', 'role' => 'search', 'autocomplete' => 'off')
        ); ?>
    ...
    <?php echo $this->Form->create('PageSize', array(
            'url' => array('controller' => 'states', 'action' => 'index', 'admin' => true), 'class' => 'form', 'autocomplete' => 'off', 'id' => 'pageSizeForm')
    ); ?>
            ...
            <table class="table table-striped table-bordered table-hover table-full-width">
                ...
                <?php if (!empty($result_data)) {
                    ...
                    foreach ($result_data as $data) { ?>
                        <tr>
                            <td><?php echo ucfirst($data['StateLocale']['name']); ?></td>
                            <td><?php echo $languages[$data['StateLocale']['lang_code']]; ?></td>
                            <td><?php echo $countries[$data['State']['country_id']]; ?></td>
                            <td> <?php
                                if ($data['State']['status'] == 'A') {
                            ...
                            <td>
                                <div>
                                    <?php
                                    if ($data['State']['status'] == 'A') {
                                        echo $this->Html->link('<i class="fa fa-check-circle"></i>', array('controller' => 'states', 'action' => 'status', base64_encode($data['State']['id']), 'I', 'admin' => true), array('title' => 'Click here to inactive', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                    } else {
                                        echo $this->Html->link('<i class="fa fa-times-circle"></i>', array('controller' => 'states', 'action' => 'status', base64_encode($data['State']['id']), 'A', 'admin' => true), array('title' => 'Click here to active', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                    }

                                    echo $this->Html->link('<i class="fa fa-pencil"></i>',
                                        array('plugin' => false, 'controller' => 'states', 'action' => 'add_edit', 'id' => base64_encode($data['State']['id']), 'admin' => true),
                                        array('class' => 'btn btn-transparent btn-xs', 'title' => 'Click here to edit State', 'escape' => false)
                                    );
                                    ?>
                                </div>
                            </td>
                    ...
                    ?>
                    <tr>
                        <td colspan="5" class="text-center">State Not Available !</td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
