...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        jQuery('#reset_button').click(function(){
            ...
            jQuery('#order_by').val('Country.country_code ASC, CountryLocale.name ASC');
        });
...
$option_order = array(
    'Country.country_code ASC, CountryLocale.name ASC' => 'Country Name Ascending',
    'Country.country_code DESC, CountryLocale.name DESC' => 'Country Name Descending',
    'Country.status ASC, Country.country_code ASC' => 'Status Ascending',
    'Country.status DESC, Country.country_code ASC' => 'Status Descending'
);
...
<section id="page-title">
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <h1 class="mainTitle pull-left">Country List</h1>
            <div class="row pull-right">
                <?php
                echo $this->Html->link('Add Country <i class="fa fa-plus"></i>',
                    array('plugin' => false, 'controller' => 'countries', 'action' => 'add_edit', 'admin' => true),
...
<div class="container-fluid container-fullw bg-white overflow-x">
    ...
    <div class="border-around margin-bottom-15 padding-10">
        <?php echo $this->Form->create('Country', array(
                'url' => array('controller' => 'countries', 'action' => 'index', 'admin' => true), 'class' => 'form', 'role' => 'search', 'autocomplete' => 'off')
        ); ?>
    ...
    <?php echo $this->Form->create('PageSize', array(
            'url' => array('controller' => 'countries', 'action' => 'index', 'admin' => true), 'class' => 'form', 'autocomplete' => 'off', 'id' => 'pageSizeForm')
    ); ?>
            ...
            <table class="table table-striped table-bordered table-hover table-full-width">
                ...
                <?php if (!empty($result_data)) { ?>
                    <?php foreach ($result_data as $data) { ?>
                        <tr>
                            <td><?php echo ucfirst($data['CountryLocale']['name']); ?></td>
                            <td><?php echo $languages[$data['CountryLocale']['lang_code']]; ?></td>
                            <td><?php echo $data['Country']['country_code']; ?></td>
                            <td> <?php
                                if ($data['Country']['status'] == 'A') {
                            ...
                            <td>
                                <div>
                                    <?php
                                    if ($data['Country']['status'] == 'A') {
                                        echo $this->Html->link('<i class="fa fa-check-circle"></i>', array('controller' => 'countries', 'action' => 'status', base64_encode($data['Country']['id']), 'I', 'admin' => true), array('title' => 'Click here to inactive', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                    } else {
                                        echo $this->Html->link('<i class="fa fa-times-circle"></i>', array('controller' => 'countries', 'action' => 'status', base64_encode($data['Country']['id']), 'A', 'admin' => true), array('title' => 'Click here to active', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                    }

                                    echo $this->Html->link('<i class="fa fa-pencil"></i>',
                                        array('plugin' => false, 'controller' => 'countries', 'action' => 'add_edit', 'id' => base64_encode($data['Country']['id']), 'admin' => true),
                                        array('class' => 'btn btn-transparent btn-xs', 'title' => 'Click here to edit Country', 'escape' => false)
                                    );
                                    ?>
                                </div>
                            </td>
                    ...
                    ?>
                    <tr>
                        <td colspan="5" class="text-center">Country Not Available !</td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
