...
$option_order = array(
    ...
    'User.customer_status ASC, User.first_name ASC, User.last_name ASC' => 'Status Ascending',
    'User.customer_status DESC, User.first_name ASC, User.last_name ASC' => 'Status Descending',
...
<section id="page-title">
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <h1 class="mainTitle pull-left">Customer List</h1>
            <div class="row pull-right">
                <?php
                    echo $this->Html->link('Add Customer <i class="fa fa-plus"></i>',
                        array('plugin' => false, 'controller' => 'customers', 'action' => 'add_edit', 'admin' => true),
...
<div class="container-fluid container-fullw bg-white">
    ...
    <div class="border-around margin-bottom-15 padding-10">
        <?php echo $this->Form->create('User', array(
                'url' => array('controller' => 'customers', 'action' => 'index', 'admin' => true), 'class' => 'form', 'role' => 'search', 'autocomplete' => 'off')
        ); ?>
    ...
    <?php echo $this->Form->create('PageSize', array(
            'url' => array('controller' => 'customers', 'action' => 'index', 'admin' => true), 'class' => 'form', 'autocomplete' => 'off', 'id' => 'pageSizeForm')
    ); ?>
                ...
                <?php if (!empty($result_data)) { ?>
                    <?php foreach ($result_data as $data) { ?>
                        <tr>
                            ...
                            <td>
							<?php //echo ucfirst($data['User']['first_name']) . ' ' . ucfirst($data['User']['last_name']); ?>
							
							<?php
							$dirvername = ucfirst($data['User']['first_name']) . ' ' . ucfirst($data['User']['last_name']); 
							 echo $this->Html->link($dirvername,array('plugin' => false, 'controller' => 'customers', 'action' => 'view', 'id' => base64_encode($data['User']['id']), 'admin' => true),array('class' => '', 'title' => 'Click here to view User', 'escape' => false));
							?>
							
							
							</td>
                            <td><a href="mailto:<?php echo $data['User']['email']; ?>"><?php echo $data['User']['email']; ?></a></td>
                            <td><?php echo $data['User']['phone']; ?></td>
                            <td> <?php
                                if ($data['User']['customer_status'] == 'A') {
                            ...
                            <td>
                                <div>
                                    <?php
                                        if ($data['User']['customer_status'] == 'A') {
                                            echo $this->Html->link('<i class="fa fa-check-circle"></i>', array('controller' => 'customers', 'action' => 'status', base64_encode($data['User']['id']), 'I', 'admin' => true), array('title' => 'Click here to inactive', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                        } else {
                                            echo $this->Html->link('<i class="fa fa-times-circle"></i>', array('controller' => 'customers', 'action' => 'status', base64_encode($data['User']['id']), 'A', 'admin' => true), array('title' => 'Click here to active', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                        }

                                        echo $this->Html->link('<i class="fa fa-pencil"></i>',
                                            array('plugin' => false, 'controller' => 'customers', 'action' => 'add_edit', 'id' => base64_encode($data['User']['id']), 'admin' => true),
                                            array('class' => 'btn btn-transparent btn-xs', 'title' => 'Click here to edit customer', 'escape' => false)
                                        );
										
										echo $this->Html->link('<i class="fa fa-eye"></i>',
                                            array('plugin' => false, 'controller' => 'customers', 'action' => 'view', 'id' => base64_encode($data['User']['id']), 'admin' => true),
                                            array('class' => 'btn btn-transparent btn-xs', 'title' => 'Click here to view customers', 'escape' => false)
                                        );
										
                                        echo $this->Html->link('<i class="fa fa-key"></i>',
                                            '/admin/customers/change_password/' . base64_encode($data['User']['id']),
                    ...
                    ?>
                    <tr>
                        <td colspan="7" class="text-center">Customer Not Available !</td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
