...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        /// driver license approval update
		
		jQuery('a[id ^= update_approval_]').click(function () {
            var thisID = $(this).attr('id');
            var breakID = thisID.split('_');
            var record_id = breakID[2];
            swal({
                title: "Are you sure?",
                text: "You wanna Update Approval Status!",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: '#DD6B55',
                confirmButtonText: 'Yes, Update Approval!',
                closeOnConfirm: false,
            },
            function () {
                jQuery.ajax({
                    type: 'get',
                     url: '<?php echo $this->Html->url(array('plugin' => false, 'controller' => 'drivers', 'action' => 'admin_update_approval')) ?>',
                    data: 'id=' + record_id,
                    dataType: 'json',
                    success: function (data) {
                        if (data.succ == '1') {
                            swal({
                                title: " Updated Approved!",
                                text: data.msg,
                                type: "success",
                                showCancelButton: false,
                                confirmButtonColor: '#d6e9c6',
...
<section id="page-title">
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <h1 class="mainTitle pull-left">Driver List </h1>
            <div class="row pull-right">
                <?php
                    echo $this->Html->link('Add Driver <i class="fa fa-plus"></i>',
                        array('plugin' => false, 'controller' => 'drivers', 'action' => 'add_edit', 'admin' => true),
...
<div class="container-fluid container-fullw bg-white">
    ...
    <div class="border-around margin-bottom-15 padding-10">
        <?php echo $this->Form->create('User', array(
                'url' => array('controller' => 'drivers', 'action' => 'index', 'admin' => true), 'class' => 'form', 'role' => 'search', 'autocomplete' => 'off')
        ); ?>
    ...
    <?php echo $this->Form->create('PageSize', array(
            'url' => array('controller' => 'drivers', 'action' => 'index', 'admin' => true), 'class' => 'form', 'autocomplete' => 'off', 'id' => 'pageSizeForm')
    ); ?>
        ...
        <div class="col-md-12">
            <table class="table table-striped table-bordered table-hover table-full-width">
                ...
                <?php if (!empty($result_data)) { ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <div>
                                    <?php
                                        if ($data['User']['driver_status'] == 'A') {
                                            echo $this->Html->link('<i class="fa fa-check-circle"></i>', array('controller' => 'drivers', 'action' => 'status', base64_encode($data['User']['id']), 'I', 'admin' => true), array('title' => 'Click here to inactive', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                        } else {
                                            echo $this->Html->link('<i class="fa fa-times-circle"></i>', array('controller' => 'drivers', 'action' => 'status', base64_encode($data['User']['id']), 'A', 'admin' => true), array('title' => 'Click here to active', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                        }
                                        ...
                                        if(!empty($data['DriverDetail']['approve'])){
                                            if($data['DriverDetail']['approve'] === 'N'){
                                              
                                                echo $this->Html->link('<i class="fa fa-check-square-o"></i>
                                        ', 'javascript:void(0)', array('class' => 'btn btn-transparent btn-xs tooltips', 'tooltip-placement' => 'top', 'tooltip' => 'Approval', 'id' => 'update_approval_' . $data['User']['id'], 'escape' => false));
                                            }else{
                                                echo $this->Html->link('<i class="fa fa-check-square-o" style="color:red;"></i>
                                        ', 'javascript:void(0)', array('class' => 'btn btn-transparent btn-xs tooltips', 'tooltip-placement' => 'top', 'tooltip' => 'Approval', 'id' => 'update_approval_' . $data['User']['id'], 'escape' => false));
                    ...
                    ?>
                    <tr>
                        <td colspan="9" class="text-center">Driver Not Available !</td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
