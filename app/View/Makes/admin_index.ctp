...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        jQuery('#reset_button').click(function(){
            ...
            jQuery('#order_by').val('MakeLocale.name ASC');
        });
...
$option_order = array(
    'MakeLocale.name ASC' => 'Name Ascending',
    'MakeLocale.name DESC' => 'Name Descending',
    'Make.status ASC, MakeLocale.name ASC' => 'Status Ascending',
    'Make.status DESC, MakeLocale.name ASC' => 'Status Descending',
    'Make.created ASC' => 'Created On Ascending',
    'Make.created DESC' => 'Created On Descending',
);
...
<section id="page-title">
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <h1 class="mainTitle pull-left">Vehicle Make List</h1>
            <div class="row pull-right">
                <?php
                echo $this->Html->link('Add Make <i class="fa fa-plus"></i>',
                    array('plugin' => false, 'controller' => 'makes', 'action' => 'add_edit', 'admin' => true),
...
<div class="container-fluid container-fullw bg-white">
    ...
    <div class="border-around margin-bottom-15 padding-10">
        <?php echo $this->Form->create('Make', array(
                'url' => array('controller' => 'makes', 'action' => 'index', 'admin' => true), 'class' => 'form', 'role' => 'search', 'autocomplete' => 'off')
        ); ?>
    ...
    <?php echo $this->Form->create('PageSize', array(
            'url' => array('controller' => 'makes', 'action' => 'index', 'admin' => true), 'class' => 'form', 'autocomplete' => 'off', 'id' => 'pageSizeForm')
    ); ?>
            ...
            <table class="table table-striped table-bordered table-hover table-full-width">
                ...
                <?php if (!empty($result_data)) { ?>
                    <?php foreach ($result_data as $data) { ?>
                        <tr>
                            <td><?php echo ucfirst($data['MakeLocale']['name']); ?></td>
                            <td><?php echo $languages[$data['MakeLocale']['lang_code']]; ?></td>
                            <td> <?php
                                if ($data['Make']['status'] == 'A') {
                            ...
                            <td>
                                <?php echo date(DATETIME_FORMAT, $data['Make']['created']); ?>
                            </td>
                            <td>
                                <div>
                                    <?php
                                    if ($data['Make']['status'] == 'A') {
                                        echo $this->Html->link('<i class="fa fa-check-circle"></i>', array('controller' => 'makes', 'action' => 'status', base64_encode($data['Make']['id']), 'I', 'admin' => true), array('title' => 'Click here to inactive', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                    } else {
                                        echo $this->Html->link('<i class="fa fa-times-circle"></i>', array('controller' => 'makes', 'action' => 'status', base64_encode($data['Make']['id']), 'A', 'admin' => true), array('title' => 'Click here to active', 'escape' => false, 'class' => 'btn btn-transparent btn-xs'));
                                    }

                                    echo $this->Html->link('<i class="fa fa-pencil"></i>',
                                        array('plugin' => false, 'controller' => 'makes', 'action' => 'add_edit', 'id' => base64_encode($data['Make']['id']), 'admin' => true),
                                        array('class' => 'btn btn-transparent btn-xs', 'title' => 'Click here to edit Make', 'escape' => false)
                                    );
                                    ?>
                                </div>
                            </td>
                    ...
                    ?>
                    <tr>
                        <td colspan="5" class="text-center">Make Not Available !</td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
