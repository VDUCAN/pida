     ...
     */
		...
		$this->autoRender = false;
			...
			$cityval_id  = $this->data['cityval_id'];
			 ...
			 $userdata = $this->User->find('all',array('conditions'=>array('User.country_id'=>$country_id,'User.state_id'=>$stateval_id,'User.city_id'=>$cityval_id,'user_type'=>array('D','B'))));
	...
	//	$userdata = $this->User->find('all',array('conditions'=>array('user_type'=>array('N','B'))));
	        ...
	        $a = array();
			 foreach($userdata as $parr) {
                ...
                /*
	        $a = array();
		    $classdata = $this->User->find('all', array('joins' => array(
					array(
						'table' => 'user_location_logs',
						'alias' => 'UserLocationLog',
						'type' => 'INNER',
						'conditions' => array('User.id = UserLocationLog.user_id')
					),
				 ),
				 'conditions'=>array('User.country_id'=>$country_id,'User.state_id'=>$stateval_id,'User.city_id'=>$cityval_id,'User.user_type'=>'D'),
				 'fields' => array('UserLocationLog.*','User.*'),
			     //'order' => array('Temp.created' => 'DESC'),
			     //'limit' =>3
			));
			//pr($classdata);die;
			
			
			foreach($classdata as $parr) {
				 if($parr['UserLocationLog']['lat'] != '') {

					 //$image = (!empty($parr['User']['photo'])) ? USER_IMAGE_STORE_HTTP_PATH . $parr['User']['photo'] : '';
					  $a[] = array(
						  'lat' => $parr['UserLocationLog']['lat'],
						  'lng' => $parr['UserLocationLog']['long'],
						  'name' => ucfirst($parr['User']['first_name']) . ' ' . ucfirst($parr['User']['last_name']),
						  //'image' => $image,
						  //'mobile'=>(!empty($parr['User']['phone'])) ? $parr['User']['phone'] : ''
...
//            $userdata = $this->User->find('all',array('conditions'=>array('User.country_id'=>$country_id,'User.state_id'=>$stateval_id,'User.city_id'=>$cityval_id,'user_type'=>array('N','B'))));
		...
		if($this->request->is('ajax')){
			...
			//pr($citydata);die;
			$citylist = $this->CityLocale->find('list',array('conditions'=>array('city_id'=>$citydata,'lang_code'=>'en')),array('fields'=>array('id','name')));
			//pr($citylist);die;
