...
//								return true;	
	...
	$query = "SELECT *
		    ...
		    pickup_date <  (UNIX_TIMESTAMP() + ($cronJobDuration * 60))
	...
	$query = "SELECT `driver_details`.`id` AS `driver_detail_id` "
		...
		. " WHERE last_login < (UNIX_TIMESTAMP() - ($cronJobDurationAutoLogout * 60))"
    ...
    private function sendNotificationAndroid($andriodPushData, $deviceToken) {
        ...
        $API_KEY = ANDRIOD_PUSH_KEY;
        $url = 'https://fcm.googleapis.com/fcm/send';

        /* if ($noti_type == 'book_ride') {
            $sound = 'bookington.aac';
        } else {
            $sound = 'default';
        } */
		$sound = 1;
        ...
        $fields = array(
            ...
            'data' => array(
                "title" => 'Pida',
                "message" => $andriodPushData,
                "body" => isset($andriodPushData['push_msg'])?$andriodPushData['push_msg']:"",
                "sound" => $sound,
            )
        );
	...
	$passphrase = IPHONE_PASS_PHRASE;
  	$pemFile = APIPATH . 'v1/' . 'apns-pro.pem';
	if (strlen($deviceToken) > 62) {
...
//	    $message['title'] = $message['push_msg'];
	...
	if (mysql_num_rows($res) > 0) {
		...
		$driverAmt = $this->getDriverAmt($array['price']);
	...
	$query = "SELECT
		  ...
		  FROM
		    users AS u
		  WHERE u.device_token != '$deviceToken' AND u.id = $id;";  
	...
	$id = $params['id'];
	$query = "SELECT
		    b.id,b.pickup_date as pickup_date,d.id as driver_id,vt.image as vehicle_type_image,d.first_name,d.last_name,d.phone,d.photo,u.device_type,u.device_token,vtl.name,v.plate_no
		  FROM
		    bookings AS b
		    LEFT JOIN users AS u
		      ON b.user_id = u.id
		  ...
		  WHERE b.id = $id AND u.device_token != '';";  
	...
	$query = "SELECT bl.source_lat,bl.source_long,bl.booking_id,bl.destination_lat,bl.destination_long"
		. " FROM"
		. " booking_locations AS bl where id=$id"; 
    ...
    public function sendSMTPmail($to , $subject, $message,  $senderName=""){
		...
		try {
			...
			$mail->Host = 'mail.pida.com';  	   // Specify main and backup SMTP servers
			$mail->SMTPAuth = true;            // Enable SMTP authentication
			$mail->Username = 'admin@pida.com';   // SMTP username
			$mail->Password = 'c0]GZ9!jg;81';   // SMTP password
    ...
    public function customerRegistration($data = array()) {
		...
		if ($this->checkUserFieldUnique('email', $email)) {
			...
			$this->createCustomerStripeAccount($response);
			$this->createDriverStripeAccount($response);
...
//	$this->runCronJob();
	...
	if (!empty($params['pickup_date'])) {
	    ...
	    $pickupDate = strtotime(gmdate('Y-m-d H:i:s'));
	    $bookingType = 1;
	}
...
//		$this->sendBookingPush(array('id'=>32)); 
     ...
     */
     public function runCronJobDaily() {
     	...
     	$arrivedPayment = "SELECT id FROM bookings WHERE pickup_date <= UNIX_TIMESTAMP() - INTERVAL 2 DAY and booking_status IN (1,4,2);";
	...
	if (mysql_num_rows($chk_user) > 0) {
	    ...
	    if ($isUpdated) {
			$fields = array('users.id',
				...
				'IFNULL(users.country_id, \'0\') country_id',
				'IFNULL(users.state_id, \'0\') state_id',
				'IFNULL(users.city_id, \'0\') city_id',
				'IFNULL(zip_code, \'\') zip_code',
				'IFNULL(photo, \'\') photo',
				'IFNULL(dob, \'\') dob',
				'IFNULL(stripe_rider_id, \'\') stripe_rider_id',
				'IFNULL(stripe_driver_id, \'\') stripe_driver_id',
				'IFNULL(cl.name, \'\') country_name',
				'IFNULL(sl.name, \'\') state_name',
				'IFNULL(ctl.name, \'\') city_name'
			);
			$fieldsSql = implode(',', $fields);
			$selectSql = "select $fieldsSql from users LEFT JOIN country_locales as cl on (users.country_id = cl.country_id AND cl.lang_code = 'en') LEFT JOIN state_locales as sl on (users.state_id = sl.state_id AND sl.lang_code = 'en') LEFT JOIN city_locales as ctl on (users.city_id = ctl.city_id AND ctl.lang_code = 'en') where users.id=$driverId";
	...
	if (!empty($insurancePolicyDoc)) {
	    ...
	    if($fileSize > $maxFileSize){ //echo 'FileSize'.$fileSize.'MaxFileSize'.$maxFileSize;
	...
	$queryBooking = "SELECT  IFNULL(driver.first_name, '') first_name,IFNULL(driver.last_name, '') last_name,bookings.id,IFNULL(bookings.pickup_date, '') pickup_date,bookings.price,IFNULL(bookings.total_miles, '') total_miles,bookings.id as booking_id "
			. "FROM bookings "
			. "LEFT JOIN users as driver ON bookings.driver_id = driver.id "
			. "WHERE bookings.booking_status IN (1,2,4) AND bookings.user_id = $userId AND bookings.pickup_date > (UNIX_TIMESTAMP() - (10 * 60)) "
...
//		);
	...
	$queryPayment = "SELECT bookings.pickup_date AS transaction_date,transactions.driver_amount AS price,IFNULL(bookings.total_miles, '') total_miles,bookings.id as booking_id, booking_locations.source_address as source_address, booking_locations.destination_address as destination_address FROM transactions
			...
			WHERE transactions.status IN ('1','2') AND bookings.driver_id = $driverId ORDER BY bookings.pickup_date DESC";
	...
	// TO-Do need to change transactions.created to bookings.pickup_date
	$queryPayment = "SELECT transactions.`status` as transaction_status ,WEEK(transactions.created) week,sum(transactions.driver_amount) as price,GROUP_CONCAT(transactions.id separator ',') as transaction_ids,
			 ...
			 DATE_FORMAT(DATE_ADD(transactions.created, INTERVAL(7-DAYOFWEEK(transactions.created)) DAY),'%Y-%m-%e')) AS DateRange
		         ...
		         GROUP BY YEARWEEK(transactions.created) ORDER BY transactions.created DESC";
	$resPayment = mysql_query($queryPayment);
	...
	if (mysql_num_rows($resPayment) > 0) {
		...
		$response[] = array(
		    ...
		    'price' => $array['price'],
		    'transaction_ids' => base64_encode($array['transaction_ids']),
    ...
    /**
     * This function used to get driver payment history details
     * @params array $params
     * @return array 
     */
    public function getDriverPaymentHistoryDetails($params = NULL) {

	$string = base64_decode($params['transaction_ids']);

	$array=array_map('intval', explode(',', $string));
    $array = implode("','",$array);

	$queryPayment = "SELECT transactions.`status` as transaction_status ,UNIX_TIMESTAMP(transactions.`created`) created ,transactions.driver_amount as price
		         FROM `transactions`
		         WHERE transactions.id IN ('".$array."') ORDER BY transactions.`created` DESC;";
	$resPayment = mysql_query($queryPayment);
...
//		    $driveAmt = $amount - $totalAdminAmt;
	...
	if($flag==0){
		...
		$bookingCancelMinutes = BOOKING_CANCEL_MINUTE;
	    $bookingSql = "SELECT * from bookings where bookings.id = $bookingId AND (UNIX_TIMESTAMP() < (bookings.pickup_date + ( $bookingCancelMinutes * 60)))  ";
	    $chkBooking = mysql_query($bookingSql);
	    if (mysql_num_rows($chkBooking) == 0) {
	...
	if($flag==0){
	    ...
	    $bookingScheduleCancelMinutes = BOOKING_SCHEDULE_CANCEL_MINUTE;
	    $bookingSql = "SELECT * from bookings where bookings.id = $bookingId AND (UNIX_TIMESTAMP() < (bookings.pickup_date + ( $bookingScheduleCancelMinutes * 60))) ;";
    ...
    $queryb = "SELECT b.id as booking_id,b.booking_status,b.price,b.pickup_lat,b.pickup_long,b.total_miles,b.price,b.dimension,b.cubicfeet,u.first_name,u.last_name,u.photo,u.phone,d.device_type,d.device_type,d.device_token,IFNULL(b.cargo_type_notes,'') cargo_type_notes,IFNULL(b.delivery_type_notes,'') delivery_type_notes,IFNULL(b.lbh_dimension,'') lbh_dimension,IFNULL(d.lang_code, '" . $langCode . "') lang_code, v.plate_no, vtl.name AS vehicle_type_name, b.pickup_date, b.pickup_time,d.first_name driver_first_name,d.last_name driver_last_name,d.photo driver_photo,d.phone driver_phone
    FROM
    bookings AS b
    LEFT JOIN users AS u
    ON b.user_id = u.id
	...
	ON b.vehicle_id=v.id
	LEFT JOIN vehicle_type_locales AS vtl 
	ON v.vehicle_type_id=vtl.vehicle_type_id AND vtl.lang_code='$langCode'
    WHERE b.id = $bookingId;"; 
	...
	if (mysql_num_rows($resb) > 0) {
	    ...
	    while ($array = mysql_fetch_assoc($resb)) {
		    ...
		    $driver_image_thumb = $driver_image_large = '';
		    if ($array['driver_photo'] != '') {
			if (file_exists(USER_PHOTO_PATH_THUMB . $array['driver_photo'])) {
			    $driver_image_thumb = USER_PHOTO_URL_THUMB . $array['driver_photo'];
			}
			if (file_exists(USER_PHOTO_PATH_LARGE . $array['driver_photo'])) {
			    $driver_image_large = USER_PHOTO_URL_LARGE . $array['driver_photo'];
			}
		    }
			...
			'image_thumb' => $image_thumb,
			'image_large' => $image_large,
			'driver_image_thumb' => $driver_image_thumb,
			'driver_image_large' => $driver_image_large,
			'booking_cargo_types' => $bookingCargoTypes,
			'booking_delivery_types' => $bookingDeliveryTypes,
			'cargo_type_notes' => $array['cargo_type_notes'],
			'delivery_type_notes' => $array['delivery_type_notes'],
			...
			'cubicfeet' => $array['cubicfeet'],
			'phone' => $array['phone'],
			'driver_phone' => $array['driver_phone'],
			'device_token' => $array['device_token'],
			'device_type' => $array['device_type'],
			'lbh_dimension' => $array['lbh_dimension'],
			'plate_no' => $array['plate_no'],
	...
	if (!empty($bank)) {
		...
		$result = $gateway->merchantAccount()->update($subMerchantId,['funding' => ['accountNumber' => $accountNo,'routingNumber' => $routerNo]]);
...
if ($result->success) {
  ...
  echo $result->errors;
	...
	$query = "SELECT `Booking`.`id` as booking_id,`Booking`.`booking_status` as booking_status, `Booking`.`booking_type` as booking_type, `Booking`.`total_miles` as booking_total_miles,`Booking`.`price` as booking_price,`Transaction`.`driver_amount` as driver_amount,`User`.`email` as user_email, `User`.`first_name` as user_first_name, `User`.`last_name` as user_last_name, `User`.`phone` as user_phone,`Driver`.`email` as driver_email, `Driver`.`first_name` as driver_first_name, `Driver`.`last_name` as driver_last_name, `Driver`.`phone` as driver_phone,`Vehicle`.`plate_no` as vehicle_plate_no FROM `pida_pida`.`bookings` AS `Booking`  LEFT JOIN `transactions` AS `Transaction` ON (`Transaction`.`booking_id` = `Booking`.`id`) LEFT JOIN `users` AS `User` ON (`User`.`id` = `Booking`.`user_id`) LEFT JOIN `users` AS `Driver` ON (`Driver`.`id` = `Booking`.`driver_id`) LEFT JOIN `vehicles` AS `Vehicle` ON (`Booking`.`vehicle_id` = `Vehicle`.`id`) WHERE `Booking`.`id` = $bookingId LIMIT 1";
	...
	if (mysql_num_rows($res) > 0) {
		...
		$template = file_get_contents(APIPATH."v1/invoice_email.html");
			...
			$driverTemplate = file_get_contents(APIPATH."v1/invoice_email.html");

			$variables['booking_price'] = $variables['driver_amount'];

	
			foreach ($variables as $key => $value) {
				$driverTemplate = str_replace('{{ ' . $key . ' }}', $value, $driverTemplate);
			}
	    }
...
//	    die;
	...
	//    $driverEmail = 'jaswant@brsoftech.org';
	//    $riderEmail = 'jspanwarbwr@gmail.com';

	//    echo $driverMsg;die;
	     $this->sendSMTPmail($riderEmail, $sub, $message);
		   $this->sendSMTPmail($driverEmail, $sub, $driverMsg);
    ...
    public function refresh_token($data = array()) {

		$userId = $data['user_id'];
		$device_type = $data['device_type'];
		$device_token = $data['device_token'];

		$query = "SELECT u.*,cl.name as country_name,sl.name as state_name,ctl.name as city_name, (select count(*) from bank_details bd where bd.user_id=u.id) as totbankaccount, (select count(*) from credit_cards cc where cc.user_id=u.id) as totcards FROM users u LEFT JOIN country_locales as cl on (u.country_id = cl.country_id AND cl.lang_code = 'en') LEFT JOIN state_locales as sl on (u.state_id = sl.state_id AND sl.lang_code = 'en') LEFT JOIN city_locales as ctl on (u.city_id = ctl.city_id AND ctl.lang_code = 'en') WHERE u.id = '{$userId}' LIMIT 1";
		$chkusr = mysql_query($query);
		$datusr = mysql_fetch_assoc($chkusr);
		$curr_time = time();

		$params = array('id'=>$datusr['id'],'device_token'=>$device_token);
		$this->sendAlreadyLoginPush($params);
		$update_user = "UPDATE users SET device_token = '{$device_token}',device_type = '{$device_type}',modified = '{$curr_time}' WHERE id='{$userId}' LIMIT 1";
		mysql_query($update_user);
		return $datusr;
	...
	public function saveStripeResponse($text, $res_type, $customer_id){
		...
		$myfile = fopen($filename, "a") or die("Unable to open file!");
		$txt = "<!---------------------[" . date("Y/m/d h:i:s") . "] $res_type::".(print_r($customer_id, true))." ----------------------->" . PHP_EOL . $text . '<!------------------------------End-------------------------------->'. PHP_EOL;
		fwrite($myfile, $txt);
		fclose($myfile);
	}
	
	################### NEW PRIVATE FUNCTIONS ADDED BY ABHISHEK 2018-04-12 #########################
	
	private function createCustomerStripeAccount($user_data){
		//including stripe library
		require_once STRIPE_AUTOLOAD;
		\Stripe\Stripe::setApiKey(STRIPE_SECRET_KEY);
		try{
			$account = \Stripe\Customer::create(array(
						  "description" => "Customer for ".$user_data["email"],
			...
			$stripe_rider_id = $stripe_res["id"];
			$update_rider = mysql_query("UPDATE users SET stripe_rider_id='$stripe_rider_id' WHERE id=".$user_data["user_id"]);
			return $stripe_rider_id;
		}
		catch(\Stripe\Error\Base $e){
			$err_message = $e->getMessage();
			$text = print_r($e->getMessage()."\r\n", true);
			$this->saveStripeResponse($text, "FAILURE", $user_data["user_id"]);
			return array("message"=>$err_message);
		}
	}	
	
	private function createDriverStripeAccount($user_data){
		//including stripe library
		require_once STRIPE_AUTOLOAD;
		\Stripe\Stripe::setApiKey(STRIPE_SECRET_KEY);
		try{
			...
			$update_driver = mysql_query("UPDATE users SET stripe_driver_id='$stripe_driver_id' WHERE id=".$user_data["user_id"]);
			return $stripe_driver_id;
		}
		catch(\Stripe\Error\Base $e){
		   
			$err_message = $e->getMessage();
			$text = print_r($err_message."\r\n", true);
			$this->saveStripeResponse($text, "FAILURE", $user_data["user_id"]);
			return array("message"=>$err_message);
		}
	}
	
	private function updateUserStripeID($stripe_id, $user_id, $stripe_type){
		if(is_string($stripe_id)){
			$update_rider = mysql_query("UPDATE users SET $stripe_type='$stripe_id' WHERE id=$user_id");
		}
		else{
			return 'UNABLE_TO_PROCEED';
		}
	...
	private function getStateNameById($state_id, $lang_code="en"){
	   
		$query = "SELECT name FROM state_locales WHERE state_id=$state_id AND lang_code=$lang_code LIMIT 1";
		$exeQuery = mysql_query($query);
		if($exeQuery && mysql_num_rows($exeQuery)>0){
			$fetchRes = mysql_fetch_assoc($exeQuery);
			return $fetchRes["name"];
		}
		return "";
	}
	
	private function getCityNameById($city_id, $lang_code="en"){
		$query = "SELECT name FROM city_locales WHERE city_id=$city_id AND lang_code=$lang_code LIMIT 1";
		$exeQuery = mysql_query($query);
		if($exeQuery && mysql_num_rows($exeQuery)>0){
			$fetchRes = mysql_fetch_assoc($exeQuery);
			return $fetchRes["name"];
	...
	private function getCountryNameById($country_id, $lang_code="en"){
		$query = "SELECT name FROM country_locales WHERE country_id=$country_id AND lang_code=$lang_code LIMIT 1";
		$exeQuery = mysql_query($query);
		if($exeQuery && mysql_num_rows($exeQuery)>0){
			$fetchRes = mysql_fetch_assoc($exeQuery);
			return $fetchRes["name"];
	...
	private function getCountryCodeById($country_id){
		$query = "SELECT country_code FROM countries WHERE id=$country_id LIMIT 1";
		$exeQuery = mysql_query($query);
		if($exeQuery && mysql_num_rows($exeQuery)>0){
			$fetchRes = mysql_fetch_assoc($exeQuery);
			return $fetchRes["country_code"];
		}
		return "";
	}
		
	private function getFullDriverDetails($driver_id, $lang_code="en"){
		$query = "SELECT u.*, dd.`ssn`, dd.`driving_license_no`, dd.`driving_license_doc`, cl.`name` country_name, c.`country_code`, sl.`name` state_name, ctl.`name` city_name
					FROM users u 
					LEFT JOIN driver_details dd
					ON u.`id`=dd.`user_id`
					LEFT JOIN countries c
					ON u.`country_id`=c.`id`
					LEFT JOIN country_locales cl
					ON u.`country_id`=cl.`country_id` AND cl.`lang_code`='$lang_code'
					LEFT JOIN state_locales sl
					ON u.`state_id`=sl.`state_id` AND sl.`lang_code`='$lang_code'
					LEFT JOIN city_locales ctl
					ON u.`city_id`=ctl.`city_id` AND ctl.`lang_code`='$lang_code'
					WHERE u.`id`=$driver_id AND u.`user_type` IN ('D','B') LIMIT 1";
		$exeQuery = mysql_query($query);
		if($exeQuery && mysql_num_rows($exeQuery)>0){
			$fetchRes = mysql_fetch_assoc($exeQuery);
			return $fetchRes;
	...
	private function getDriverBankDetails($driver_id, $lang_code="en"){
		$query = "SELECT bd.*, cl.`name` country_name, c.`country_code`, sl.`name` state_name, ctl.`name` city_name
					FROM bank_details bd
					LEFT JOIN countries c
					ON bd.`country_id`=c.`id`
					LEFT JOIN country_locales cl
					ON bd.`country_id`=cl.`country_id` AND cl.`lang_code`='$lang_code'
					LEFT JOIN state_locales sl
					ON bd.`state_id`=sl.`state_id` AND sl.`lang_code`='$lang_code'
					LEFT JOIN city_locales ctl
					ON bd.`city_id`=ctl.`city_id` AND ctl.`lang_code`='$lang_code'
					WHERE bd.`user_id`=$driver_id LIMIT 1";
		$exeQuery = mysql_query($query);
		if($exeQuery && mysql_num_rows($exeQuery)>0){
			$fetchRes = mysql_fetch_assoc($exeQuery);
			return $fetchRes;
	...
	private function getFullRiderDetails($rider_id, $lang_code="en"){
		$query = "SELECT u.*, cl.`name` country_name, c.`country_code`, sl.`name` state_name, ctl.`name` city_name
					FROM users u
					LEFT JOIN countries c
					ON u.`country_id`=c.`id`
					LEFT JOIN country_locales cl
					...
					LEFT JOIN city_locales ctl
					ON u.`city_id`=ctl.`city_id` AND ctl.`lang_code`='$lang_code'
					WHERE u.`id`=$rider_id AND u.`user_type` IN ('N','B') LIMIT 1";
		$exeQuery = mysql_query($query);
		if($exeQuery && mysql_num_rows($exeQuery)>0){
			$fetchRes = mysql_fetch_assoc($exeQuery);
			return $fetchRes;
	...
	private function changeCardsDefaultStatus($user_id, $card_id_to_skip){
		$query = "UPDATE credit_cards SET is_default='N' WHERE user_id=$user_id AND id!=$card_id_to_skip";
		$exeQuery = mysql_query($query);
		return true;
	}
	
	############# NEW API FUNCTIONS ADDED BY ABHISHEK 2018-04-12 ##############
		
	public function addBankDetails($params, $lang_code){
		//current date and time
		$date = date("Y-m-d H:i:s", time());
		//increasing time execution
	//	ini_set("max_execution_time", 0);
		//fetching driver's details
		$fetchDriverDetails = $this->getFullDriverDetails($params["user_id"]);
	
		//check if driver exists
		if(empty($fetchDriverDetails)){
			return INVALID_USER;
		}
		//check if driver is verified
		if($fetchDriverDetails["driver_status"]!="A"){
			return DRIVER_ACCOUNT_DEACTVATED;
		}

		$fetchDriverDetails["user_id"] = $fetchDriverDetails["id"];
		$stripe_driver_id = $fetchDriverDetails["stripe_driver_id"];
		//checking if driver is already registered on Stripe or not.
	
		if(empty($stripe_driver_id)){
		    	$stripe_driver_id = $this->createDriverStripeAccount($fetchDriverDetails);
		
		
// 		return SUCCESSFULLY_DONE;
			//if still driver account is not created on Stripe, we cannot proceed further.
			if(!is_string($stripe_driver_id)){
				return array("msg"=>$stripe_driver_id["message"]);
			}
		}
	
		//now fetching basic details
		$state_name = $this->getStateNameById($params["state_id"]);
	
		$city_name = $this->getCityNameById($params["city_id"]);
		
		$country_code = $this->getCountryCodeById($params["country_id"]);
		$address = $params["address"];
		$zip_code = $params["zip_code"];
		$dobArr = !empty($params["dob"])?explode("-", $params["dob"]):array();
		//saving ssn number
		$params["personal_id_number"] = $fetchDriverDetails["ssn"];
		//now fetching bank account details from DB.
		
		$bank_details = $this->getDriverBankDetails($params["user_id"]);
		//including stripe library
	
		require_once STRIPE_AUTOLOAD;
		\Stripe\Stripe::setApiKey(STRIPE_SECRET_KEY);
		
		//checking if driver's bank details are already there or not
		if(empty($bank_details) || empty($bank_details["stripe_bank_account_id"])){
			//echo "inside not-found bank details";die;
			try{
				//retrieving driver's account details
				$account = \Stripe\Account::retrieve($stripe_driver_id);
		
				//saving date of birth
				$account->individual->dob = array(
					"day" => (isset($dobArr[2])?$dobArr[2]:""),
					"month" => (isset($dobArr[1])?$dobArr[1]:""),
					"year" => (isset($dobArr[0])?$dobArr[0]:"")
				);
					
				//saving address information
				$account->individual->address = array(
					"city" => $city_name,
				...
				//saving address information
			//	$account->individual->type = "individual";
				//saving payout settings
				/*
				$account->payouts->schedule = array(
					"delay_days"=>"7",
					"interval"=>"weekly",
					"weekly_anchor"=>GLOBAL_PAYOUT_DAY
				);*/
				//trying to save personal ID number if required
				if(!empty($fetchDriverDetails["ssn"])){
				// 	$account->individual->personal_id_number = $fetchDriverDetails["ssn"];
				// 	$account->individual->ssn_last_4 = substr($fetchDriverDetails["ssn"], -4);	
				}
				//trying to save the document if there is any.
				if(!empty($fetchDriverDetails["driving_license_doc"])){
					
					try{
				// 		$fp = fopen(DRIVER_DOC_PATH_LARGE.$fetchDriverDetails["driving_license_doc"], 'r');
				...
				// 		$this->saveStripeResponse($text, "FILE_UPLOAD", $fetchDriverDetails["id"]);
				// 		$params["stripe_document_id"] 	= $stripe_document_res_arr["id"];
				// 		$params["driving_license_doc"]	= $fetchDriverDetails["driving_license_doc"];
				// 		$params["driving_license_no"] 	= $fetchDriverDetails["driving_license_no"];
				// 		//we got the document, so we'll pass it
				// 		$account->individual->verification = array(
				// 			"document" => $params["stripe_document_id"]
				// 		);
					}
					catch(\Stripe\Error\Base $e){
				...
				$account->save();
				//now saving the bank details
				$new_account = $account->external_accounts->create(array(
					"external_account" => array(
						...
						"routing_number" => $params["router_no"],
						"account_holder_name" => trim($fetchDriverDetails["first_name"]." ".$fetchDriverDetails["last_name"]),
					)
				));
				$new_account_arr = $new_account->__toArray(true);
				
				//retrieving driver's updated account details
				/* $updated_account_res = \Stripe\Account::retrieve($stripe_driver_id);
				$updated_account_arr = $updated_account_res->__toArray(true); */
				$params["stripe_bank_account_id"] = $new_account_arr["id"];
				$params["modified"] = $date;
				if(empty($bank_details)){
					$params["created"] = $date;
					$bank_data_id = $this->insertBankDetails($params);
				}
				else{
					$bank_data_id = $this->updateBankDetails($params,$bank_details["id"]);
				}
				return SUCCESSFULLY_DONE;
			}
			catch(\Stripe\Error\Base $e){
			...
			//echo "inside found bank details";die;
			try{
				//retrieving driver's account details
				//checking if bank details were updated
				if($params["account_no"]!=$bank_details["account_no"] || $params["router_no"]!=$bank_details["router_no"]){
					$account = \Stripe\Account::retrieve($stripe_driver_id);
					//now we will add one
					$new_account = $account->external_accounts->create(array(
						"external_account" => array(
					...
					$new_account_arr = $new_account->__toArray(true);
					$params["stripe_bank_account_id"] = $new_account_arr["id"];
					//we have to delete the old account				
					$account->external_accounts->retrieve($bank_details["stripe_bank_account_id"])->delete();
				}
				$params["modified"] = $date;
				$bank_data_id = $this->updateBankDetails($params,$bank_details["id"]);
				return SUCCESSFULLY_DONE;
	...
	public function addCreditCardStripe($params, $lang_code){
		//validating card expiry date
		if(preg_match("/^(0[1-9]|1[0-2])\/[0-9]{4}$/", $params["expiry_date"])!=1){
			return INVALID_DATE;
		}
		else{
			$expires = \DateTime::createFromFormat('mY', str_replace("/", "", $params["expiry_date"]));
			$now     = new \DateTime();
			if($expires < $now)
				return INVALID_DATE;
		}
		$user_details = $this->getFullRiderDetails($params["user_id"]);
		//checking if user is present in db or not
		if(empty($user_details)){
			return INVALID_USER;
		}
		$user_details["user_id"] = $params["user_id"];
		//saving rider stripe id into a variable
		$stripe_rider_id = $user_details["stripe_rider_id"];
		//checking if rider has an account on stripe or not.
		if(empty($stripe_rider_id)){
			//since it's not, so we will create one for them.
			$stripe_rider_id = $this->createCustomerStripeAccount($user_details);
			if(!is_string($stripe_rider_id)){
				return array("msg"=>$stripe_rider_id["message"]);
			}
		}
		//current date and time
		$date = date("Y-m-d H:i:s", time());
		//now we can proceed with creating a card on stripe first
		//including stripe library
		require_once STRIPE_AUTOLOAD;
		\Stripe\Stripe::setApiKey(STRIPE_SECRET_KEY);
		try{
			$expiry_date_arr = explode("/", $params["expiry_date"]);
			$expiry_month = $expiry_date_arr[0];
			$expiry_year = $expiry_date_arr[1];
			//we will generate token first.
			...
			$token_arr = $token->__toArray(true);
			$text = print_r($token_arr, true);
			
			$this->saveStripeResponse($text, "SUCCESS", $user_details["user_id"]);
			$card_token = $token_arr['id'];
			try{
				$card = \Stripe\Customer::retrieve($stripe_rider_id);
				$new_card = $card->sources->create(array("source" => $card_token));
				
				$card_arr = $new_card->__toArray(true);
				$text = print_r($card_arr, true);
				
				$this->saveStripeResponse($text, "SUCCESS", $user_details["user_id"]);
				
				$card_id = $card_arr["id"];
				
				$sql = "INSERT INTO credit_cards(user_id, cardholdername, customerid, expirationdate, number, token, is_default, created) VALUE(".$user_details["user_id"].", '".$params["card_holder_name"]."', '".$user_details["stripe_rider_id"]."', '".$params["expiry_date"]."', '".(str_pad($card_arr["last4"], 12, "*", STR_PAD_LEFT))."', '$card_id', '".$params["is_default"]."', '$date')";
				...
				if($insert_id>0){
					...
					$data = array("token"=>$card_id);
					
					if($params["is_default"]=="Y"){
						//since this card has been set to default, we have to update other credit cards of this user
						$this->changeCardsDefaultStatus($params["user_id"], $insert_id);
						$userSaveData = array('credit_card_id'=>$insert_id);
						$updateUser = $this->updateUsers($userSaveData,$params["user_id"]);
					}
					return $data;
				}
				else{
				...
				$message = $e->getMessage();
				$text = print_r($message."\r\n", true);
				$this->saveStripeResponse($text, "FAILURE", $params["user_id"]);
				return array("msg"=>$message);
			}
		}
		catch(\Stripe\Error\Base $e){
	 ...
	 public function deduct_booking_payment($param=NULL){
        date_default_timezone_set('UTC');
        $updtd = date('Y-m-d H:i:s');
        ...
        $bookingSql = "SELECT bookings.id,bookings.price,bookings.status,credit_cards.token,transactions.id,bookings.driver_id, users.stripe_rider_id
						FROM bookings
						LEFT JOIN users ON bookings.user_id = users.id
						LEFT JOIN credit_cards ON users.credit_card_id = credit_cards.id
						LEFT JOIN transactions ON transactions.booking_id = bookings.id
      ...
      //  die($payment_method_token);
        if (!empty($amount) && !empty($payment_method_token)) {
            
            if($bookingData['status']=='3'){
                 return SUCCESSFULLY_DONE;
            }
            //we'll try to capture the payment
            //including stripe library
            require_once STRIPE_AUTOLOAD;
            \Stripe\Stripe::setApiKey(STRIPE_SECRET_KEY);
            try{
                $charge = \Stripe\Charge::create(array(
                        ...
                        "currency"=> 'usd',
                        "customer"=> $bookingData["stripe_rider_id"],
                        "card"=> $payment_method_token
                    )
                );
                $charge_response = $charge->__toArray(true);

                $text = print_r($charge_response, true);

                $this->saveStripeResponse($text, "BOOKING PAYMENT SUCCESS", $bookingData);
                ...
                $remaingAmt = $amount - $pidaFee;

                $adminAmt = $remaingAmt * $pidaCommission/100;

                $totalAdminAmt = $pidaFee + $adminAmt;
                $driveAmt = $amount - $totalAdminAmt;
                ...
                $transaction_id = $charge_response["balance_transaction"];
                $charge_id = $charge_response["id"];
                $is_captured = intval($charge_response["captured"]);

                $transactionData = array('booking_id' => $bookingId, 'charge_id' => $charge_id, 'transaction_id' => $transaction_id, 'created' => $updtd, 'is_captured'=>$is_captured, 'status'=>'1', 'driver_amount'=>$driveAmt,'admin_amount'=>$adminAmt, 'admin_percent'=>$pidaCommission, 'admin_pida_fee'=>$pidaFee, 'admin_total_amount'=>$adminAmtPaypl);
                $transaction_id = $this->insertTransactions($transactionData);

                $updateData = array('status'=>'3','transaction_id'=>$transaction_id);
                $this->updateBookings($updateData,$bookingId);
                ...
                $text = print_r($e->getMessage()."\r\n", true);
                $this->saveStripeResponse($text, "BOOKING PAYMENT FAILURE", $bookingId);
                ...
                $updateData = array('status'=>'2','booking_status'=>'3','transaction_id'=>$transaction_id);
                $result = $this->updateBookings($updateData,$bookingId);

                // sending push notificatio to driver about calcellation
                $this->notify_driver_about_cancellation($bookingId);

                return array("msg"=>$rejectReason);
            }
        } else {
            $updateData = array('status'=>'2','booking_status'=>'3');
             $this->updateBookings($updateData,$bookingId);
            // sending push notificatio to driver about calcellation
            $this->notify_driver_about_cancellation($bookingId);
            return NO_RECORD_FOUND;
	...
	private function notify_driver_about_cancellation($bookingId){
        $query = "SELECT b.*,d.device_token,d.device_type"
            . " FROM"
            . " bookings AS b"
            . " LEFT JOIN users AS d on b.driver_id = d.id"
            . " WHERE b.id = $bookingId AND d.device_token != '';";
    ...
    public function bookingCompleted($param=NULL){
		date_default_timezone_set('UTC');
		$updtd = date('Y-m-d H:i:s');
		...
		$bookingSql = "SELECT bookings.id,bookings.price,credit_cards.token,transactions.id,bookings.driver_id, users.stripe_rider_id
						FROM bookings
						LEFT JOIN users ON bookings.user_id = users.id
						LEFT JOIN credit_cards ON users.credit_card_id = credit_cards.id
						LEFT JOIN transactions ON transactions.booking_id = bookings.id
		...
		if (!empty($amount) && !empty($payment_method_token)) {
			...
			try{
				...
				$updateData = array('booking_status'=>'5','travel_route'=>$travel_route);
				$this->updateBookings($updateData,$bookingId);
	...
	public function bookingCompleted_bk($param=NULL){
		date_default_timezone_set('UTC');
		$updtd = date('Y-m-d H:i:s');
	...
	//get driver details 
	function get_driver_details($user_id){
		$query = "SELECT * from driver_details where user_id = '".$user_id."'";
		$query_result = mysql_query($query);
		$result = mysql_fetch_assoc($query_result);
		return $result;
	}	
	//save customer profile image 
	function upload_customer_image($customer_id,$image){
		$query = "INSERT INTO customer_details(`customer_id`,`image`) VALUES('".$customer_id."','".$image."')";
		$query_result = mysql_query($query);
		if($query_result){
	...
	function get_user_data($user_id){
		...
		$query_result_data = mysql_query($query);
		$query_result = mysql_fetch_assoc($query_result_data);

		$data['first_name'] = $query_result['first_name'];
		$data['last_name'] = $query_result['last_name'];  
		if($query_result['user_type'] == 'N'){
			$get_customer_image_qry = "SELECT * from customer_details where customer_id='".$user_id."'";
			$get_customer_image_result_exec = mysql_query($get_customer_image_qry);
			$get_customer_image_result = mysql_fetch_assoc($get_customer_image_result_exec);
			if($get_customer_image_result['image'] !=''){
				$data['image'] = '../app/webroot/uploads/customer_documents/'.$get_customer_image_result['image'];	
			}
		}else if($query_result['user_type'] == 'D'){
			$get_user_doc_qry = "SELECT * from driver_details where user_id='".$user_id."'";
			$get_user_doc_result_data_exec = mysql_query($get_user_doc_qry);
			$get_user_doc_result = mysql_fetch_assoc($get_user_doc_result_data_exec);
			if($get_user_doc_result['driving_license_doc'] !=''){
				$data['doc'] = '../app/webroot/uploads/driver_documents/'.$get_user_doc_result['driving_license_doc']; 
			}
		}else{
			$get_customer_image_qry = "SELECT * from customer_details where customer_id='".$user_id."'";
			$get_customer_image_result_data = mysql_query($get_customer_image_qry);
			$get_customer_image_result = mysql_fetch_assoc($get_customer_image_result_data);
			if($get_customer_image_result['image'] !=''){
				$data['customer_image'] = '../app/webroot/uploads/customer_documents/'.$get_customer_image_result['image'];	
			}
			$get_user_doc_qry = "SELECT * from driver_details where user_id='".$user_id."'";
			$get_user_doc_result_exec = mysql_query($get_user_doc_qry);
			$get_user_doc_result = mysql_query($get_user_doc_result_exec);
			if($get_user_doc_result['image'] !=''){
				$data['doc'] = '../app/webroot/uploads/driver_documents/'.$get_user_doc_result['image']; 
			}
		}
		return $data;
	}
	############# WORK BY ABHISHEK ENDS ############################ 
}
