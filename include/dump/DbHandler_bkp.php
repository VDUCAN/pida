...
//	$post = print_r($_POST, true);
	...
	$booking_res = mysqli_query($this->conn, $select_request);
	...
	if (!empty($booking)) {
	    ...
	    $bookingRequestResult = mysqli_query($this->conn, $selectBookingRequest);
	    if (mysqli_num_rows($bookingRequestResult) > 0) {
		while ($bookingRequestData = mysqli_fetch_assoc($bookingRequestResult)) {
	...
	if (mysqli_num_rows($res) > 0) {
	    $driverDetails = mysqli_fetch_assoc($res);
	}
	...
	if (!empty($bookingAddressJson)) {
		...
		$result = mysqli_query($this->conn, $saveBookingLocationsSql);
	    }
	    if (!empty($result)) {
	...
	if (!empty($bookingCargoJson)) {
		...
		$result = mysqli_query($this->conn, $saveBookingCargoSql);
	    }
	    if (!empty($result)) {
		return true;
	    }
	...
	if (!empty($bookingDeliveryJson)) {
		...
		$result = mysqli_query($this->conn, $saveBookingDeliverySql);
	    }
	    if (!empty($result)) {
		return true;
	    }
...
//	    pr($result);die;
	...
	$updatedId = mysqli_query($this->conn, $updateBookingLocationsSql);
	return $updatedId;
    }
	...
	if (mysqli_num_rows($res) > 0) {
		...
		$bookingRequestResult = mysqli_query($this->conn, $saveBookingRequest);

		if ($bookingRequestResult) {
		    $bookingRequestInsertedId = mysqli_insert_id($this->conn);
		    if (!empty($bookingRequestInsertedId)) {
			$deleteRequest = "DELETE FROM booking_request_temps WHERE id = '{$bookingRequestTempId}'";
			$deleteResult = mysqli_query($this->conn, $deleteRequest);
...
//								return true;	
	...
	//	print_r($booking);
		...
		//    echo $saveBookingRequest;
		    $result = mysqli_query($this->conn, $saveBookingRequest);
		    if ($result) {
	...
	if (mysqli_num_rows($res) > 0) {
	    while ($driverDetails = mysqli_fetch_assoc($res)) {
     ...
     * @return boolean
     */
    private function pushNotification($data = NULL, $pushNotiyType = NULL) { 
	if ($pushNotiyType == BOOKING_REQUEST_DRIVER) {
		...
		if ($pdata['device_type'] == 'I') {
		    ...
		    $andriodPushData['push_msg'] = 'You have new booking request';
		...
		if ($pdata['device_type'] == 'I') { 
		    ...
		    $andriodPushData = array();
		    $andriodPushData = $pdata;
		    $andriodPushData['push_msg'] = 'Driver has accepted the booking request';
		...
		if ($pdata['device_type'] == 'I') { 
		    ...
		    $andriodPushData = $pdata;
		    $sourceAddress = $pdata['source_address'];
		    $andriodPushData['push_msg'] = "Driver has picked up from $sourceAddress";
		...
		if ($pdata['device_type'] == 'I') { 
		    ...
		    $andriodPushData = $pdata;
		    $destinationAddress = $pdata['destination_address'];
		    $andriodPushData['push_msg'] = "Driver has drop off to $destinationAddress";
		...
		if ($pdata['device_type'] == 'I') { 
		    ...
		    $andriodPushData = array();
		    $andriodPushData = $pdata;
		    $andriodPushData['push_msg'] = 'No driver is available for your last booking';
		...
		if ($pdata['device_type'] == 'I') { 
		    ...
		    $andriodPushData = $pdata;
		    $price = $pdata['price'];
		    $andriodPushData['push_msg'] = "Your have received an amount of $price for booking";
		...
		if ($pdata['device_type'] == 'I') { 
		    ...
		    $andriodPushData = array();
		    $andriodPushData = $pdata;
		    $andriodPushData['push_msg'] = "User has cancelled booking";
		...
		if ($pdata['device_type'] == 'I') { 
		    ...
		    $andriodPushData = array();
		    $andriodPushData = $pdata;
		    $andriodPushData['push_msg'] = "You had login from other device";
...
//	    $message['title'] = $message['push_msg'];
     ...
     * @return boolean 
     */
    private function sendBookingPush($params = NULL) {

	$id = $params['id'];
	$query = "SELECT
					  brt.id,brt.booking_id,b.price,b.pickup_lat,b.pickup_long,b.total_miles,b.price,b.dimension,b.cubicfeet,u.first_name,u.last_name,u.photo,d.device_type,d.device_type,d.device_token
					FROM
	...
	if (mysqli_num_rows($res) > 0) {
		...
		$this->pushNotification($response, BOOKING_REQUEST_DRIVER);
	    }
	}
	return false;
    }
	...
	$query = "SELECT
		    u.device_token,u.device_type,u.email,u.user_type
		  FROM
		    users AS u
	...
	if (mysqli_num_rows($res) > 0) {
		...
		$this->pushNotification($response, ALREADY_LOGIN_NOTIFY_USER_DRIVER);
	    }
	}
	return false;
    }
	...
	$id = $params['id'];
	$query = "SELECT
		    b.id,b.pickup_date,d.id as driver_id,vt.image as vehicle_type_image,d.first_name,d.last_name,d.phone,d.photo,u.device_type,u.device_token,vtl.name,v.plate_no
		  FROM
		    bookings AS b
		    LEFT JOIN users AS u
		      ON b.user_id = u.id
	...
	if (mysqli_num_rows($res) > 0) {
		...
		$this->pushNotification($response, BOOKING_REQUEST_ACCEPT_USER);
	    }
	}
	return false;
    }
     ...
     * @return boolean 
     */
    private function sendLocationPush($params = NULL) {

	$id = $params['id'];
	$type = $params['type'];
	$query = "SELECT bl.*,u.device_token,u.device_type"
		. " FROM"
	...
	if (mysqli_num_rows($res) > 0) {
		...
		if($type=='S'){
		    $this->pushNotification($response, BOOKING_LOCATION_SOURCE_USER);
		}else if($type=='D'){
		    $this->pushNotification($response, BOOKING_LOCATION_DESTINATION_USER);
		}
	    }
	}
	return false;
	...
	if (mysqli_num_rows($res) > 0) {
		...
		$this->pushNotification($response, BOOKING_REQUEST_REJECT_USER);
	    }
	}
	return false;
    }
	...
	$sel_user = "SELECT id  from users WHERE email = '$email' AND (password = '$md5Password' OR password='$password')";
...
/*	} elseif ($this->checkDriverDetailFieldUnique('ssn', $ssn)) {
	    ...
	    $save_user = "INSERT INTO users 
							(first_name, last_name, phone, email, password, created, modified, user_type, customer_status, driver_status) 
							VALUES 
							('$first_name', '$last_name', '$phone', '$email', '$encode_password','$curr_time', '$curr_time', 'D', 'I', 'I')";
...
//
//		mysqli_query($this->conn, $save_detail);
...
//		$response['driving_license_no'] = '' . $driving_license_no;
	...
	if (in_array($login_from, $login_types)) {
	    ...
	    $query = "SELECT u.*, (select count(*) from bank_details bd where bd.user_id=u.id) as totbankaccount, (select count(*) from credit_cards cc where cc.user_id=u.id) as totcards FROM users u WHERE u.email = '{$email}' LIMIT 1";
		...
		if (!empty($datusr) && $datusr['password'] == md5($password)) {
			...
			if ('A' == $datusr['driver_status']) {
			    ...
			    $get_data = mysqli_query($this->conn, $query1);
			    $datusr['DriverDetail'] = mysqli_fetch_assoc($get_data);
...
//							
			...
			if ($insertId > 0) {
			    $usrdat = mysqli_query($this->conn, "SELECT * FROM users WHERE id = {$insertId}");
			    if (mysqli_num_rows($usrdat) > 0) {
				$datusr = mysqli_fetch_assoc($usrdat);
				$msg = "Your Login Id : " . $email . " AND  Password : " . $npassword;
				@mail($email, "Pida app login detail", $msg, "From :info@pidaapp.com");
				return $datusr;
			    }
			} else {
			    return UNABLE_TO_PROCEED;
	...
	$resCountry = mysqli_query($this->conn, $queryCountry);

	if (mysqli_num_rows($resCountry) > 0) {
	    
	    $arrayCountry = mysqli_fetch_assoc($resCountry);
	...
	if (!empty($user)) {
	    ...
	    mysqli_query($this->conn, $save_log);
	    ...
	    $result = mysqli_affected_rows($this->conn);		
			...
			if($booking_id) {
				...
				if(!empty($array)) {
					...
					mysqli_query($this->conn, $UdateBooking);
	...
	if (!empty($booking)) {
		...
		if (!empty($bookingRequestInsertedId)) {
		    ...
		    $vechicle_res = mysqli_query($this->conn, $select_vechicle);
		    $vechicle = mysqli_fetch_assoc($vechicle_res);
...
//		$this->sendBookingPush(array('id'=>1));
...
//	$this->autoLogoutDriver();
	...
	$response = array();
	$db = new DbHandler();
	$chk_user = mysqli_query($this->conn, "select * from users where id=$userId AND user_type IN('N','B') AND customer_status = 'A' ");
	...
	if (mysqli_num_rows($chk_user) > 0) {
	    ...
	    if ($image != '') {
		$user_image = mysqli_query($this->conn, "select photo from users where id=" . $userId);
	    ...
	    $result = mysqli_affected_rows($this->conn);
	    if ($result > 0) {
		$chk_user = mysqli_query($this->conn, "select * from users where id=" . $userId);
	...
	$response = array();
	$db = new DbHandler();
	$chk_user = mysqli_query($this->conn, "select count(*) from users where id=$driverId AND user_type IN('D','B') AND driver_status = 'A' ");
	if (mysqli_num_rows($chk_user) > 0) {
	    $pass_data = mysqli_fetch_array($chk_user);
	    ...
	    if ($image != '') {
		$user_image = mysqli_query($this->conn, "select photo from users where id=" . $driverId);
		$uimagedata = mysqli_fetch_array($user_image);
		//echo $uimagedata['image']; exit;
		if (isset($image['name']) && $image['name'] != '') {
		    $ex1 = explode(".", $image['name']);
		...
		$selectDriver = mysqli_query($this->conn, $selectSql);
		$driverData = mysqli_fetch_array($selectDriver);
		return $driverData;
	    } else {
		return UNABLE_TO_PROCEED;
	    }
	...
	$vehicleResult = mysqli_query($this->conn, $vehicleSql);
	$vehiclesData = mysqli_fetch_assoc($vehicleResult);
	...
	$vehiclesResult = mysqli_query($this->conn, $vehiclesSql);
	if (mysqli_num_rows($vehiclesResult) == 0) {
	        return NO_RECORD_FOUND;
	}

	$driverDetailsSql = "SELECT id FROM driver_details WHERE user_id = '$driverId'"; 
	$driverDetailsResult = mysqli_query($this->conn, $driverDetailsSql);
	$driverDetailsData = mysqli_fetch_assoc($driverDetailsResult);
	...
	$resPayment = mysqli_query($this->conn, $queryPayment);
	
	if (mysqli_num_rows($resPayment) > 0) {

	    $response = array();
	    while ($array = mysqli_fetch_assoc($resPayment)) {
	...
	$resBooking = mysqli_query($this->conn, $queryBooking);
	
	if (mysqli_num_rows($resBooking) > 0) {

	    $response = array();
	    while ($array = mysqli_fetch_assoc($resBooking)) {
...
//		);
	...
	$resTotal = mysqli_query($this->conn, $queryTotal);
	$dataTotal = mysqli_fetch_assoc($resTotal);
	
	$total = (!empty($dataTotal['total']) ? $dataTotal['total'] : 0);
	
	$queryPayment = "SELECT transactions.created AS transaction_date,transactions.driver_amount AS price,IFNULL(bookings.total_miles, '') total_miles,bookings.id as booking_id
			FROM transactions
			LEFT JOIN bookings ON transactions.booking_id = bookings.id
	...
	if ($this->checkDriverDetailFieldUnique('ssn', $ssn, $driverId)) {
	    ...
	    $resultdd = mysqli_query($this->conn, $querydd);
	    $num_rows_dd = mysqli_num_rows($resultdd);
	    
	    if($num_rows_dd){	
		$dd = mysqli_fetch_assoc($resultdd);
		$dd_id = $dd['id'];
		...
		$driverData = array('dob'=>$dob);
		$this->updateUsers($driverData,$userId);
...
/*	    if($userData['user_type'] == 'D'){
	...
	$bookingLocationResult = mysqli_query($this->conn, $bookingLocationSql);
	$bookingLocationData = mysqli_fetch_assoc($bookingLocationResult); 
	...
	$chkusr = mysqli_query($this->conn,$userSql);
...
//		$res = mysqli_query($this->conn, $update_user);
//	    }
	}
	...
	if ($result->success) {
	    ...
	    $chkcard = mysqli_query($this->conn,"select id from credit_cards where uniquenumberidentifier='$uniquenumberidentifier'");
	    
	    if (mysqli_num_rows($chkcard)) {
...
//	    mysql_query($save_card);
	...
	$bookingSql = "SELECT booking_locations.driver_arrived_source,bookings.id
			...
			WHERE bookings.id = $bookingId AND bookings.booking_status = 4 AND (booking_locations.driver_arrived_source ='N' OR booking_locations.driver_arrived_destination ='N')";  // echo $bookingSql;die;
	
	$chkBooking = mysqli_query($this->conn,$bookingSql);
	
	if (mysqli_num_rows($chkBooking) > 0) {
...
//	$flag = 0;
//	if (mysqli_num_rows($chkBooking) > 0) {
//	    while ($array = mysqli_fetch_assoc($chkBooking)) {
...
//	}
	...
	$chkBooking = mysqli_query($this->conn,$bookingSql);
	$bookingData = mysqli_fetch_assoc($chkBooking);
	...
	if(!empty($amount) && !empty($payment_method_token) && empty($id)){ 
	    ...
	    $resb = mysqli_query($this->conn, $queryb);

	    if (mysqli_num_rows($resb) > 0) {
		$bankArr = mysqli_fetch_assoc($resb); 
	...
	if (mysqli_num_rows($bookingResult)) {
	    
	    $ratingSql = "SELECT r.id FROM ratings as r where r.booking_id = $bookingId";
	    $ratingResult = mysqli_query($this->conn, $ratingSql);
	    
	    if(mysqli_num_rows($ratingResult)){
	...
	$rescc = mysqli_query($this->conn, $querycc);
	
	if (mysqli_num_rows($rescc) > 0) {
	    $response = array();
	    while ($array = mysqli_fetch_assoc($rescc)) {
		$response[] = $array;
	    }
	    return $response;
	} else {
	...
	if (!empty($cc)) {
	    ...
	    $res = mysqli_query($this->conn, $update_cc);
	...
	if(!empty($id)){
	    ...
	    $user_res = mysqli_query($this->conn, $update_user);
	...
	$queryb = "SELECT * 
		    FROM `bookings` as b
		    WHERE b.user_id = $userId AND b.id=$bookingId ";
	$resb = mysqli_query($this->conn, $queryb);
	
	if (mysqli_num_rows($resb) > 0) {
	    $response = array();
	    while ($array = mysqli_fetch_assoc($resb)) {
		$response[] = $array;
	    }
	    return $response;
	} else {
	...
	$bank_res = mysqli_query($this->conn, $select_bank);
...
if ($result->success) {
  ...
  echo $result->errors;
    ...
    public function reportaProblem($data = array()) {
	    ...
	    if(mysqli_query($this->conn, $insert_report)) {
	        return SUCCESSFULLY_DONE;
		} else {
			return UNABLE_TO_PROCEED;
		}
	...
	if (mysqli_num_rows($res) > 0) {
	    ...
	    while ($variables = mysqli_fetch_assoc($res)) {
//		echo '<pre>';
//		print_r($variables);
		...
		$template = file_get_contents("invoice_email.html");
...
//	    $riderEmail = 'jspanwarbwr@gmail.com';
	    ...
	    @mail($driverEmail, $sub, $message, $headers);
	    @mail($riderEmail, $sub, $message, $headers);
