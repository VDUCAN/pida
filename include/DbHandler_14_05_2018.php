...
//		    $driveAmt = $amount - $totalAdminAmt;
	...
	if($flag==0){
	    ...
	    $bookingSql = "SELECT * from bookings where bookings.id = $bookingId AND bookings.pickup_date > UNIX_TIMESTAMP() + INTERVAL -15 MINUTE";
	    $chkBooking = mysql_query($bookingSql);
	    if (mysql_num_rows($chkBooking) == 0) {
	...
	if($flag==0){
	    ...
	    $bookingSql = "SELECT * from bookings where bookings.id = $bookingId AND (bookings.pickup_date - (15 * 60)) > UNIX_TIMESTAMP();";
