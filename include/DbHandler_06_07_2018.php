...
//		print_r($variables);
		...
		/* $template_keys = array_keys($variables);
		$template_values = array_values($variables); */
...
//	    die;
	...
	public function addBankDetails($params, $lang_code){
		...
		//print_r($fetchDriverDetails);die;
		...
		if(empty($bank_details) || empty($bank_details["stripe_bank_account_id"])){
			...
			try{
				...
				if(!empty($fetchDriverDetails["ssn"])){
					$account->legal_entity->personal_id_number = $fetchDriverDetails["ssn"];
					$account->legal_entity->ssn_last_4 = substr($fetchDriverDetails["ssn"], -4);	
				}
