...
//								return true;	
	...
	if ($pushNotiyType == BOOKING_REQUEST_DRIVER) {
		...
		if ($pdata['device_type'] == 'I') { 
		    ...
		    //$iphonePushData['push_msg'] = "Driver has picked up from $sourceAddress";
		    $iphonePushData['push_msg'] = getMessage($lang_code, "REACHED_DRIVER").$destination_address;
		    ...
		    $andriodPushData = $pdata;
		    $destination_address = $pdata['destination_address'];
		    $andriodPushData['push_msg'] = getMessage($lang_code, "REACHED_DRIVER").$destination_address;
...
//		$response['driving_license_no'] = '' . $driving_license_no;
		...
		@mail($email, $sub, $message, $headers);
		@mail('info@pida.com', $sub2, $message2, $headers);
...
//	    mysql_query($save_card);
    ...
    public function bookingComplete($param=NULL){
		...
		$bookingSql = "SELECT booking_locations.driver_arrived_source,bookings.id
				...
				WHERE bookings.id = $bookingId AND bookings.booking_status = 4 AND (booking_locations.driver_arrived_source ='N' OR booking_locations.driver_arrived_destination ='N')";
		// echo $bookingSql;die;
